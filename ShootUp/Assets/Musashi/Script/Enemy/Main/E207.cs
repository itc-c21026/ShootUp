...
public class E207 : MonoBehaviour
{
    GameObject Player;
    ...
    IEnumerator Shot()
    {
        ...
        Enemy_Bullet = Instantiate(Bullet, Attackobj.transform.position, Attackobj.transform.rotation);
        Enemy_Bullet.GetComponent<EnemyBullet>().Damage = float.Parse(Mystatus[2]);
        yield return new WaitForSeconds(0.08f);
        Enemy_Bullet = Instantiate(Bullet, Attackobj.transform.position, Attackobj.transform.rotation);
        Enemy_Bullet.GetComponent<EnemyBullet>().Damage = float.Parse(Mystatus[2]);
    ...
    public void HPCheck()
    {
        ...
        if (!Dead)
        {
            if (HP <= 0)
            {
                ...
                Destroy(downObj, 5f);
                EnemyDrop.SendMessage("MachineGunDrop");
